...
#' @param sigma Float. Noise strength.
...
#' @export
sample_toy_data <- function(n=200, dx=100, dy=500, sigma=.5, only_observations=TRUE){
  ...
  X_noise <- sigma * matrix(rnorm(n*dx), nrow = n, ncol = dx)
  
  X_obs <- 5000 * (X_joint + X_indiv + X_noise)
  
  Y_joint <- cbind(matrix(0, nrow = n, ncol = 4*dy/5),
                   rbind(matrix(1, nrow = n/2, ncol = dy/5),
  ...
  Y_noise <- sigma * matrix(rnorm(n*dy), nrow = n, ncol = dy)
  ...
  if(only_observations){
    ...
    output[['decomp']] <- list('1'=list('joint'=list('full'=X_joint),
                                        'individual'=list('full'=X_indiv),
                                        'noise'=X_noise),
                               '2'=list('joint'=list('full'=Y_joint),
                                        'individual'=list('full'=Y_indiv),
                                        'noise'=Y_noise))
