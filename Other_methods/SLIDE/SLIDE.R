...
source("./Other_methods/SLIDE/AuxillaryFunctions.R")
...
updateVoptim1 <- function(XU, lambda, pvec){
  ...
  for (i in 1:d){
    ...
    norms <- sqrt(colSums(as.matrix(XU[index, ]^2)))  ## modified
...
evaluatef_optim1 <- function(XU, V, lambda, pvec){
  ...
  XU = as.matrix(XU) ## modified
  V = as.matrix(V)   ## modified
  ...
  for (i in 1:d){
    ...
    f <- f + lambda * sum(sqrt(colSums(as.matrix(V[index,]^2))))  ## modified
  }
  return(f)
}
...
solve_optim1 <- function (X, lambda, pvec, k_max = 1000, eps = 1e-6, reduced = F, rank_total = NULL, Uinit = NULL){
  ...
  if (is.null(Uinit)){
    ...
    U <- as.matrix(X_svd$u[ , 1:rank_total])  ## modified
    if (rank_total > 1){
      V <- as.matrix(X_svd$v[ , 1:rank_total] %*% diag(X_svd$d[1:rank_total])) ## modified
    } else {
      V <- as.matrix(X_svd$v[ , 1:rank_total] * (X_svd$d[1:rank_total])) ## modified
    }
  }else{
    U = as.matrix(Uinit)  ## modified
    V = as.matrix(crossprod(X, U))  ## modified
    rank_total = ncol(U)
  }
  # Current function value
  f <- evaluatef_optim1(as.matrix(crossprod(X,U)), as.matrix(V), lambda, pvec)  ## modified
  k <- 1
  error <- 100
  while ((k < k_max)&(error[k] > eps)){
    k <- k + 1
    # Update V
    V <- as.matrix(updateVoptim1(as.matrix(crossprod(X,U)), lambda, pvec)) ## modified
    # Update U
    if (reduced){
      ...
      XV_svd <- svd(X%*%V)
      # Update U
      U <- as.matrix(tcrossprod(XV_svd$u, XV_svd$v))  ## modified
    }else{
...
compareBCV_vs_Best_structure_refit_center <- function(X, pvec, trueU, trueV, nl = 30, n_fold = 3, p_fold = 3, k_max = 1000, eps = 1e-6, reduced = F, rank_total = NULL, center = T){
  ...
  return(list(best = list(lambda = lambda_all[best_id], error = errors[best_id_used], 
                          ranks = ranks[[best_id]], best_id = best_id, best_struct = out_struct$Slist[[best_id_used]]), 
              bcv = list(lambda = lambda_all[bcv_id], error = errors[bcv_id_used], 
                         ranks = ranks[[bcv_id]], bcv_id = bcv_id, bcv_struct = out_struct$Slist[[bcv_id_used]])))
}
