...
# Jushan Bai, Serena Ng (2002), Determining the Number of Factors in Approximate Factor Models, Biometrika 70(1), 191-221.
# Sources are from https://github.com/cran/PANICr
...
getnfac <- function(x, kmax = NULL, criteria = NULL) {
  ...
  eig <- tryCatch( { eig <- svd(t(XX))},
      error = function(ee){
        if(require(svd)) eig<-propack.svd(t(XX))
      },
      finally = {
        #pass
      }
