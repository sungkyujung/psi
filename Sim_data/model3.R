...
model.generate <- function(n = 200, K = 3, p = c(100,100,100), weight = c(1,1,1), 
                           U = NULL, V.var = c(1.4, 0.8, 1.3, 0.7, 1.2, 0.6),
                           SNR = 0,  rank = c(2,2,2))
{
  ...
  r.list = c(rank[1] + rank[2], rank[1] + rank[3], rank[2] + rank[3] )
  r.index.list = get.list(rank, length(rank))
  
  r.structure = list()
  # r.structure[[1]] = c(1,2,3,4)
  # r.structure[[2]] = c(1,2,5,6)
  # r.structure[[3]] = c(3,4,5,6)
  
  r.structure[[1]] = c(r.index.list[[1]],r.index.list[[2]])
  r.structure[[2]] = c(r.index.list[[1]],r.index.list[[3]])
  r.structure[[3]] = c(r.index.list[[2]],r.index.list[[3]])
  ...
  r.U[[3]] = list(r.index.list[[2]],r.index.list[[3]])
  ...
  true.struct = matrix(c(1,1,0, 1,0,1, 0,1,1),3,3)
  true.struct = true.struct[,c(rep(1,rank[1]),rep(2,rank[2]),rep(3,rank[3]))]
