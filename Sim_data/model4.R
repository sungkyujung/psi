                           ...
                           SNR = 0,  rank = c(2,2,2,2))
{
  ...
  # List of index for r
  # For column indices, the order is  r123, r1, r2, r3
  
  r.list = c(rank[1] + rank[2], rank[1] + rank[3], rank[1] + rank[4] )
  r.index.list = get.list(rank, length(rank))
  
  r.structure = list()
  # r.structure[[1]] = c( seq(1, rank[1]), seq(rank[1]+1, rank[1]+ rank[2])  )
  # r.structure[[2]] = c( seq(1, rank[1]), seq(rank[1] + rank[2] +1, rank[1]+ rank[2] + rank[3])  )
  # r.structure[[3]] = c( seq(1, rank[1]), seq(rank[1] + rank[2] + rank[3] +1, rank[1]+ rank[2] + rank[3] + rank[4])  )
  
  r.structure[[1]] = c(r.index.list[[1]],r.index.list[[2]])
  r.structure[[2]] = c(r.index.list[[1]],r.index.list[[3]])
  r.structure[[3]] = c(r.index.list[[1]],r.index.list[[4]])
  ...
  r.U = list()
  r.U[[1]] = list(r.index.list[[1]],r.index.list[[2]])
  r.U[[2]] = list(r.index.list[[1]],r.index.list[[3]])
  r.U[[3]] = list(r.index.list[[1]],r.index.list[[4]])
  ...
  V[[1]] <- cbind(V.comp[[1]], V.comp[[2]])
  V[[2]] <- cbind(V.comp[[1]], V.comp[[3]])  
  V[[3]] <- cbind(V.comp[[1]], V.comp[[4]])
  ...
  true.struct = matrix(c(1,1,1, 1,0,0, 0,1,0, 0,0,1),3,4)
